...
class Pipe:
    ...
    async def pipe(self, body: dict, __user__: dict, __request__: Request, __event_emitter__=None, __event_call__=None):
        ...
        try:
            ...
            if last_user_message.startswith("http") or last_user_message.startswith("https"):
                ...
                user_action_input = await self.action(__event_call__, "Enter your message...", "Enter your message...", "input")
            ...
            if doc_url:
                await self.update_status("Resolving document URL...", __event_emitter__)
                # Check if we have already downloaded the file
                existing_file = self.check_existing_file(user_action_input)
                if not existing_file == False:
                    await self.update_status(f"File already exists...{existing_file}", __event_emitter__)
                    downloaded_file_path = existing_file
                else:
                    await self.update_status("Downloading file...", __event_emitter__)
                    downloaded_file_path = self.download_file(user_action_input)

                if self.is_pdf(downloaded_file_path):
                    ...
                    if os.path.exists(os.path.join("pages", doc_title)):
                        await self.update_status(f"Pages already exist...{os.path.join('pages', doc_title)}", __event_emitter__)
                        page_pngs = os.listdir(os.path.join("pages", doc_title))
                        png_files.extend(page_pngs)
                    else:
                        await self.update_status("Converting to PNGs...", __event_emitter__)
                        page_pngs = await self.convert_pdf_to_pngs(downloaded_file_path,__event_emitter__)
                        png_files.extend(page_pngs)

                elif self.is_image(downloaded_file_path):
                    await self.update_status(f"Image: {downloaded_file_path}", __event_emitter__)
                    png_files.append(downloaded_file_path)
                else:
                    await self.update_status(f"Unsupported file type: {downloaded_file_path}", __event_emitter__)
                    await self.update_response(f"Unsupported file type: {downloaded_file_path}", __event_emitter__)


                # Process PNG files
                await self.update_status("Processing PNG files...", __event_emitter__)
                user_pages = await self.action(
                    __event_call__, 
                    "Process Images", 
                ...
                if user_pages:
                    ...
                    for page_number in page_numbers:
                        await self.update_status(f"Processing page {page_number} ({current_page}/{len(page_numbers)})...", __event_emitter__)
                        try:
                            # Get the image data from the png_file as base64
                            image_path = os.path.join("pages", doc_title, f"page_{page_number}.png")
                                ...
                                messages_payload = [
                                    {
                                        ...
                                        "content": [
                                            {
                                                "type": "text",
                                                "text": "You are a helpful assistant. You will transcribe text within an image and respond with the verbatim transcription. Do not respond with any additional text."
                                            }
                                        ]
                                    },
                                    {
                                        ...
                                        "content": [
                                            {
                                                "type": "text",
                                                "text": "Transcribe the text in the image and respond with a verbatim transcription."
                                            },
                                            {
                                            "type": "image_url",
                                            "image_url": {
                            ...
                            try:
                                # await self.update_response("Sending to LLM...", __event_emitter__)
                                response = await generate_chat_completion(request=__request__, form_data=chat_data, user=user, bypass_filter=True)
                                collected_response = ""
                                    ...
                                    if chunk:
                                        ...
                                        try:
                                            ...
                                            if content := json_chunk.get('choices', [{}])[0].get('delta', {}).get('content'):
                                                ...
                                                await self.update_response(content, __event_emitter__, False)
                                        except json.JSONDecodeError:
                                            # log traceback with exception
                                            logger.error(
                                                f"JSONDecodeError in chunk: {chunk_data}"
                                            )
                                            continue
                                ...
                                await self.update_response(f"Error with LLM: {str(e)}", __event_emitter__)
                                logger.error(f"Error with LLM: {str(e)}")

                        except Exception as e:
                            # log traceback with exception
                            logger.error(f"Error processing page: {e}")
                            await self.update_response(str(e), __event_emitter__)
            else:
                await self.update_response("No document found.", __event_emitter__)

            try:
                await self.update_response("\n\n".join(llm_responses), __event_emitter__)
                pipe_success = True
            except Exception as e:
                # log traceback with exception
                logger.error(f"Error returning llm responses[]: {e}")
                await self.update_response(str(e), __event_emitter__)
                pipe_success = False
        ...
        await self.update_status(f"Pipe {pipe_success}", "successfully :)" if pipe_success else "failure :(", __event_emitter__, done=True)
    
    async def update_status(self, message: str, __event_emitter__=None, done: bool = False):
        """
    ...
    async def update_response(self, message: str, __event_emitter__=None, add_line: bool = True):
        """
    ...
    async def action(self, __event_call__=None, title: str = "Enter your message...", message: str = "Enter your message...", action_type: str = "input", placeholder: str = "Enter your message..."):
        """
    ...
    def download_file(self, url, output_folder=UPLOAD_DIR):
        """
        ...
        if "drive.google.com" in url:
            ...
            url = self.convert_onedrive_url(url)
        response = requests.get(url, stream=True)
        if response.status_code != 200:
            raise Exception(f"Failed to download file. HTTP Status Code: {response.status_code}")
        filename = self.get_filename_from_url_or_headers(url, response.headers)
        if not filename:
    ...
    def get_filename_from_url_or_headers(self, url, headers):
        """
    ...
    async def convert_pdf_to_pngs(self, pdf_path,__event_emitter__, output_folder="pages"):
        """
        ...
        page_images = convert_from_path(pdf_path, dpi=600, thread_count=1)
        page_paths = []
        for idx, image in enumerate(page_images):
            page_path = os.path.join(output_folder,title_folder, f"page_{idx + 1}.png")
            image.save(page_path, "PNG")
            await self.update_status(f"Converted page {idx + 1}/{len(page_images)}...", __event_emitter__)
    ...
    def check_existing_file(self, file_url):
        ...
        """
        try:
            filename = self.get_filename_from_url_or_headers(file_url, requests.head(file_url).headers)
