                ...
                if self.user_valves.COMPILE_TO_PDF == True:
                    ...
                    try:
                        ...
                        if new_pdf_doc == True:
                            ...
                            file_expiration = now + timedelta(minutes=self.valves.FILE_RETENTION_TIME)
                            await update_response(f"\n\nDownload PDF: {WEBUI_URL}/api/v1/files/{new_file_id}/content\n\nThis link will expire at {file_expiration.strftime('%Y-%m-%d %I:%M:%S %p %Z')}", __event_emitter__)
                        else:
                            ...
                            try:
                                if os.path.exists(path):
                                    if os.path.isfile(path):
                                        ...
                                        if os.path.getsize(path) < 1000000 and Path(path).suffix == ".pdf":  # 1MB
                                            logger.debug(f"Skipping cleanup of small file: {path}")
                                        else:
                                            # Remove it otherwise (pngs, jpgs, etc.)
                                            os.remove(path)
                                            logger.debug(f"Removed non-PDF file: {path}")
