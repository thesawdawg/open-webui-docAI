...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        MAX_STREAM_PAGES: int = Field(default=20, description="The maximum number of pages to stream.")
        FILE_RETENTION_TIME: int = Field(default=3, description="The number of minutes to retain the files generated by the tool. Smaller PDFs are retained for faster processing.")
        KEEP_PNGS: bool = Field(default=False, description="Whether to keep the PNG files after processing.")
        KEEP_COMPILED_PDF: bool = Field(default=False, description="Whether to keep the compiled PDF after processing.")


    class UserValves(BaseModel):
        """UserValves class contains the valves for the user."""
        MODEL_ID: str = Field(default="llama3.2-vision:latest")
        STREAM: bool = Field(default=True, description="Whether to stream the response from the model.")
        PROCESS_ALL_PAGES: bool = Field(default=False, description="Whether to process all pages by default.")
        ...
        INCLUDE_IMG_PDF: bool = Field(default=True, description="Whether to include the image with the transcription in the PDF.")
    ...
    async def pipe(self, body: dict, __user__: dict, __request__: Request, __event_emitter__=None, __event_call__=None, __files__=None):
        ...
        **Available commands**:
        - Enter a URL to a PDF or image. e.g., https://media.wizards.com/2018/dnd/downloads/DnD_BasicRules_2018.pdf
        - Use the upload function and attach a PDF or image
        -- I will download the file (if it is a url), convert each page to a PNG file with a dpi of {self.valves.PNG_DPI} 
           and offer what pages you would like to be transcribed. From your selection of pages (it can be all of them)
           I will analyze the page and transcribe any text I find. If your control for "Compile to PDF" is enabled,
           I will combine all the transcribed pages into a single PDF. If your control for "Include Image with Transcription" is enabled,
           I will include the image with the transcription in the PDF. The compiled PDF will be available for download for {self.valves.FILE_RETENTION_TIME} minutes.
           After the link expires...poof, all of the files will be deleted.
        ...
        async def handle_pdf(
            ...
            created_filepaths.append(str(target_dir))
            ...
            if new_pngs:
                created_filepaths.extend(new_pngs)
            return new_pngs
        ...
        async def process_url_task(
            ...
            created_filepaths.append(downloaded_path)
        ...
        async def process_file_task(
            ...
            for file in task["data"]["files"]:
                ...
                if is_pdf(file_path):
                    ...
                    clean_png = convert_unsupported_image(file_path)
                    created_filepaths.append(clean_png)
        ...
        async def process_image_task(
            ...
            for image in task["data"]["images"]:
                ...
                created_filepaths.append(str(output_path))
        ...
        def gen_combined_pdf(
            data_list, 
            output_path = self.valves.COMBINED_PDF_DIR, 
            include_images = self.user_valves.INCLUDE_IMG_PDF
            ):
            """
            ...
            try:
                ...
                pdf = fpdf.FPDF(format='letter')  # Standard 8.5x11" format
                pdf.set_auto_page_break(auto=True, margin=10)  # Enable auto page breaks
                pdf.set_margin(15)  # Set margins once at creation time
                ...
                for item in items_sorted:
                    ...
                    if include_images:
                        ...
                        pdf.multi_cell(page_width, 10, f"Page {item['page_number']}", align='c')

                        # Center the image on the page
                        image_width = (pdf.w - 20) - (pdf.l_margin + pdf.r_margin)
                        image_x = (pdf.w - image_width) / 2
                        pdf.image(png_path, x=image_x, w=image_width)
                    ...
                    # Add the page header
                    pdf.multi_cell(page_width, 10, f"Page {item['page_number']}\n",align='c')
                    
                    # Write the transcription text with automatic wrapping
                    pdf.multi_cell(page_width - 10, 10, transcription_text)
                    
                # Finally, save the PDF
                output_path = Path(self.valves.COMBINED_PDF_DIR) / (Path(png_files[0]).stem + "_transcription.pdf")
                output_path.parent.mkdir(parents=True, exist_ok=True)
                created_filepaths.append(str(output_path))
        ...
        try:
            ...
            if len(tasks) == 0 and last_user_message is not None and isinstance(last_user_message, str):
                # We couldnt figure out any tasks to perform. Lets ask the user again
            ...
            if len(tasks) == 0:
                return "Error...still no tasks...we definitely need a task here....seriously...i am amazed this has been returned. Kudos!"
            ...
            if user_pages:
                ...
                # If we have a large page count and the stream valve is set to true, verify with the user
                if page_count > 5 and self.user_valves.STREAM:
                    user_verify = await action(
                        __event_call__, 
                        "Large Page Count", 
                        f"""You have selected {page_count} pages. Streaming responses is enabled.
                        This may result in a large amount of text.
                        Do you want to stream all {page_count} pages?(Y/n)""", 
                        "input"
                    )
                    if isinstance(user_verify, str) and user_verify.lower() in ["y", "yes", ""]:
                        # Double double check the user isnt a knucklehead and about to stream 200 pages
                        if page_count > self.valves.MAX_STREAM_PAGES:
                            await update_status("Maximum allowed page stream exceeded.", __event_emitter__, True)
                            self.user_valves.STREAM = False
                        
                    else:
                        self.user_valves.STREAM = False
                    if not self.user_valves.STREAM:
                        # Double check there will be some kind of output (i.e., PDF)
                        if not self.user_valves.COMPILE_TO_PDF:
                            await update_status("No output will be generated. Cmon...", __event_emitter__, True)
                            return "You must enable PDF output if you are not streaming the pages."
                ...
                if self.user_valves.COMPILE_TO_PDF == True:
                    ...
                    output_path = Path(self.valves.COMBINED_PDF_DIR) / (doc_title + "_transcription.pdf")
                    ...
                    try:
                        ...
                        if new_pdf_doc == True:
                            # Read the pdf data to save in the data key
                            with open(str(output_path), "rb") as f:
                                pdf_data = f.read()
                            
                            # Convert the bytes to a serialized string
                            serialized_pdf = base64.b64encode(pdf_data).decode('utf-8')
                            
                            form_data = FileForm(
                                id=str(doc_id),
                                filename=doc_title + ".pdf",
                                path=str(output_path),
                                data={"content": serialized_pdf},
                            ...
                            file_expiration = cst_now + timedelta(minutes=self.valves.FILE_RETENTION_TIME)
                            await update_response(f"\n\nDownload PDF: {WEBUI_URL}/api/v1/files/{new_file_id}/content\n\nThis link will expire at {file_expiration.strftime('%m-%d-%Y %I:%M%p')}", __event_emitter__)
                        else:
            ...
            # If files were downloaded, schedule cleanup
            if created_filepaths:
                async def cleanup_files(paths: list[str], delay_minutes: int = self.valves.FILE_RETENTION_TIME):
                    try:
                            ...
                            try:
                                if os.path.exists(path):
                                    if os.path.isfile(path):
                                        # If the file is a PDF and under 10mb, keep it
                                        if self.valves.KEEP_COMPILED_PDF and Path(path).suffix == ".pdf": 
                                            logger.debug(f"Skipping cleanup of file: {path}")
                                        
                                        elif self.valves.KEEP_PNGS and Path(path).suffix == ".png": 
                                            logger.debug(f"Skipping cleanup of file: {path}")
                                        else:
                                            # Remove it otherwise (pngs, jpgs, etc.)
                                            os.remove(path)
                                            logger.debug(f"Removed file: {path}")
                                    elif os.path.isdir(path):
                                        # shutil.rmtree(path)
                                        logger.debug(f"Skipping cleanup of directory: {path}")
                ...
                # Schedule cleanup of all created files
                asyncio.create_task(cleanup_files(created_filepaths))
                logger.debug(f"Scheduled cleanup for {len(created_filepaths)} paths in {self.valves.FILE_RETENTION_TIME} minutes")
